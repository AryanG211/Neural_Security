...
model.load_state_dict(torch.load("sketch_to_image_07.pth", map_location=device))
model.eval()
...
def load_face_database():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT id, name, age, location, marital_status, criminal_history, feature_vector FROM IMAGES')
    ...
    feature_vectors = [json.loads(record[6]) for record in records]  
...
def find_similar_face(feature_vector, threshold=0.3):
    ...
    if distances[0][0] >= threshold:
        ...
        if 0 <= db_index < len(records):
            person_id, person_name, person_age, person_location, marital_status, criminal_history, _ = records[db_index]
            return {
                "id": person_id,
                "name": person_name,
                "age": person_age,
        ...
        for face in faces:
            ...
            if target_face_feature_vector is not None:
                ...
                if similarity >= 0.3:
                    ...
                    cv2.putText(frame, "Match Found", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
